...
test("rejects the forgeable legacy boolean auth cookie", async () => {
...
test("rejects protocol-relative next paths", () => {
  assert.equal(getSafeInternalNextPath("//evil.example"), "/tools");
});

test("keeps same-origin internal next paths", () => {
  assert.equal(
    getSafeInternalNextPath("/report?section=pipeline#summary"),
    "/report?section=pipeline#summary",
  );
});
