...
export function SectionBlock({ section }: { section: Section }) {
  return (
    <section className="min-w-0">
      ...
      {section.soWhat ? <SoWhat>{section.soWhat}</SoWhat> : null}
    </section>
  );
}
...
export function SoWhat({ children }: { children: ReactNode }) {
  return (
    <p
      className="mt-2.5 break-inside-avoid font-sans text-[9.5px] italic leading-snug"
      style={{ color: R.olive }}
    >
      {children}
    </p>
...
export function ValueProps() {
  ...
  return (
    <div
      className="grid shrink-0 grid-cols-4 divide-x"
...
export function TaglineBar() {
  return (
    <div
      className="flex shrink-0 items-center justify-center px-4 py-3.5"
      style={{ background: R.footer }}
    >
      <p
        className="text-center font-sans text-[10px] font-medium tracking-[0.08em]"
        style={{ color: R.ink }}
      >
        Independent Insight. Stronger Decisions.
      </p>
    </div>
  );
}
